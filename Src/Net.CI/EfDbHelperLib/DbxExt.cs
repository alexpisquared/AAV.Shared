...
public static class DbxExt // replacing DbSaveLib and all others!!! (Aug 2018  ...2021-10  ...2022-05)
{
  public static async Task<(bool success, int rowsSavedCnt, string report)> TrySaveReportAsync(this DbContext dbx, string? info = "", [CallerMemberName] string callerName = "")
  {
    ...
    try
    {
      ...
      reportOut +=
          rowsSaved == 0 ? $"No DB changes to save. " :
  ...
  {
    var sb = new StringBuilder($"{db.GetType().Name}.{db.GetType().GetHashCode()}:    {db.ChangeTracker.Entries().Count(e => e.State == EntityState.Deleted),5}  Deletes  {db.ChangeTracker.Entries().Count(e => e.State == EntityState.Added),5}  Inserts  {db.ChangeTracker.Entries().Count(e => e.State == EntityState.Modified),5}  Updates  \t");
  ...
  public static string CalcConStr<T>(IServiceProvider sp, string sqlsvr, string sqlver) => string.Format(sp.GetRequiredService<IConfigurationRoot>()[sqlver]!, sqlsvr, typeof(T).Name.Replace("Context", ""), DeplConstIpm.IpmDevDbgUser, DeplConstIpm.IpmDevDbgOpen);
