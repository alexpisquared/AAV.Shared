...
public class EvLogHelperBase //this is the latest copy as of 2023-08-18. 
{
  ...
  public void LogScrSvrBgn(int App_Ssto_Gp, string msg = "ScrSvr - Up. ScreenSaveTimeOut + GracePeriod 1 min") { try { EventLog.WriteEntry(_aavSource, $"{msg}(sec) ={App_Ssto_Gp}", EventLogEntryType.Information, 7101); } catch (Exception ex) { Trace.TraceError(ex.Message); throw; } }
  public void LogScrSvrEnd(DateTime actualIdleIeStartApp, string msg = "------------------------------------") { try { EventLog.WriteEntry(_aavSource, $"Idle since {actualIdleIeStartApp:HH:mm:ss} (=App.Start) for {DateTime.Now - actualIdleIeStartApp:h\\:mm\\:ss}    {msg}", EventLogEntryType.Information, 7102); } catch (Exception ex) { Trace.TraceError(ex.Message); throw; } }
