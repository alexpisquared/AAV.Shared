...
namespace AsLink
{
  public static partial class EvLogHelper //this is the latest copy as of Sep 9, 2021.
  {
    ...
    public static void LogScrSvrEnd(DateTime actualIdleIeStartApp, int ssTimeoutSec, string msg)
    {
      try
      {
        var dt = DateTime.Now - actualIdleIeStartApp;
        EventLog.WriteEntry(_aavSource, $"Idle since {actualIdleIeStartApp:HH:mm:ss} (=App.Start) for {dt:h\\:mm\\:ss}    {msg}", EventLogEntryType.Information, 7102);
      }
      catch (Exception ex) { ex.Log(); }
    }// AavSource finds the log named AavNewLog
