...
namespace AsLink
{
    ...
    {
        const string logoSecondaryTileId = "SecondaryTile.Logo";

        public static bool IsPinned => SecondaryTile.Exists(logoSecondaryTileId);
        ...
        public static async Task<string> PinTile(object sender)
        {
            ...
            try
            {
                ...
                var st = new SecondaryTile(logoSecondaryTileId, $"{DateTime.Now:MMM d} pinned on", "???", square150x150Logo, TileSize.Square150x150);
                ...
                //causes warning, seems redundant: st.VisualElements.Square30x30Logo = square30x30Logo;  // Like the background color, the square30x30 logo is inherited from the parent application tile by default. Let's override it, just to see how that's done.
